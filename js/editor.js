...
(function(){
  ...
  if(id){var xhr=new XMLHttpRequest();xhr.open('GET',API+'?action=get&id='+id,true);xhr.onload=function(){try{var d=JSON.parse(xhr.responseText);if(d.ok){titleInput.value=d.doc.title||'';editor.innerHTML=d.doc.content||'';updateStatus();}else toast('加载失败','error');}catch(e){toast('数据错误','error');}};xhr.send();}
  ...
  document.getElementById('btn-share').onclick=function(){save(function(){share();});};
  ...
  document.getElementById('copy-confirm').onclick=function(){
    ...
    insertAtCursor(html);document.getElementById('m-copy').style.display='none';document.getElementById('copy-text').value='';
  };
  ...
  document.getElementById('video-confirm').onclick=function(){
    ...
    var html='<div class="editor-video-wrap" style="margin:12px 0"><video class="editor-video" src="'+esc(url)+'" controls preload="metadata" style="max-width:100%;border-radius:12px;box-shadow:0 4px 16px rgba(0,0,0,.15)">您的浏览器不支持视频播放</video></div>';
  ...
  document.getElementById('audio-confirm').onclick=function(){
    ...
    var html='<div class="editor-audio-wrap" style="margin:12px 0;padding:12px;background:#f8f9fa;border-radius:12px;display:flex;align-items:center;gap:12px"><span style="font-size:28px">🎵</span><audio controls preload="none" style="flex:1;height:36px"><source src="'+esc(url)+'">您的浏览器不支持音频播放</audio></div>';
  ...
  function loadFiles(){
    ...
    x.onload=function(){try{var d=JSON.parse(x.responseText);if(d.ok){fileData=d.files;renderFileChooser();}else toast('加载失败','error');}catch(e){toast('数据错误','error');}};
    x.send();
  }
  function renderFileChooser(){
    ...
    if(all.length===0){box.innerHTML='<div style="padding:32px;text-align:center;color:#999">暂无文件</div>';return;}
    box.innerHTML='';
    for(var i=0;i<all.length;i++){
      var f=all[i];var row=document.createElement('div');row.style.cssText='display:flex;align-items:center;gap:12px;padding:10px;border-radius:10px;cursor:pointer;margin-bottom:6px;background:#fafafa';
      var thumbHtml='';
      if(f.kind==='image')thumbHtml='<img src="'+f.url+'" style="width:44px;height:44px;object-fit:cover;border-radius:8px">';
      else if(f.kind==='video')thumbHtml='<div style="width:44px;height:44px;background:linear-gradient(135deg,#667eea,#764ba2);border-radius:8px;display:flex;align-items:center;justify-content:center;color:white;font-size:18px">▶</div>';
      ...
      row.innerHTML=thumbHtml+'<div style="flex:1;min-width:0"><div style="font-size:13px;font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+esc(f.display_name)+'</div><div style="font-size:11px;color:#999">'+f.kind.toUpperCase()+' · '+fmtSize(f.size)+'</div></div><button style="padding:6px 14px;background:#1E90FF;color:white;border:none;border-radius:6px;cursor:pointer;font-size:12px">插入</button>';
      row.onmouseenter=function(){this.style.background='#eef2ff';};row.onmouseleave=function(){this.style.background='#fafafa';};
      (function(file){row.querySelector('button').onclick=function(e){e.stopPropagation();insertFileIntoEditor(file);document.getElementById('m-file').style.display='none';};})(f);
      box.appendChild(row);
    }
  }
  document.querySelectorAll('#file-tabs .ft').forEach(function(b){b.onclick=function(){document.querySelectorAll('#file-tabs .ft').forEach(function(x){x.classList.remove('active');});b.classList.add('active');fileCat=b.dataset.cat;renderFileChooser();};});

  function insertFileIntoEditor(f){
    var html='';
    if(f.kind==='image'){
      html='<div style="margin:12px 0;text-align:center"><img src="'+f.url+'" alt="'+esc(f.display_name)+'" class="editor-img" style="max-width:100%;max-height:500px;border-radius:12px;box-shadow:0 4px 16px rgba(0,0,0,.15)"><div style="font-size:12px;color:#999;margin-top:6px">'+esc(f.display_name)+'</div></div>';
    }else if(f.kind==='video'){
      html='<div class="editor-video-wrap" style="margin:12px 0"><div style="font-size:12px;color:#999;margin-bottom:4px">'+esc(f.display_name)+'</div><video class="editor-video" src="'+f.url+'" controls preload="metadata" style="max-width:100%;border-radius:12px;box-shadow:0 4px 16px rgba(0,0,0,.15)">您的浏览器不支持视频播放</video></div>';
    }else if(f.kind==='audio'){
      html='<div class="editor-audio-wrap" style="margin:12px 0;padding:12px;background:#f8f9fa;border-radius:12px;display:flex;align-items:center;gap:12px"><span style="font-size:28px">🎵</span><div style="flex:1"><div style="font-size:12px;color:#333;font-weight:500;margin-bottom:4px">'+esc(f.display_name)+'</div><audio controls preload="none" style="width:100%;height:34px"><source src="'+f.url+'">您的浏览器不支持音频播放</audio></div></div>';
    }else if(f.kind==='app'){
      var icon= f.is_app_icon?('<img src="'+f.icon+'" style="width:56px;height:56px;border-radius:14px;object-fit:cover;box-shadow:0 2px 8px rgba(0,0,0,.15)">'):'<span style="font-size:36px">📱</span>';
      html='<div class="media-card app-card" style="margin:12px 0;display:flex;align-items:center;gap:16px;padding:14px;border-radius:14px;box-shadow:0 2px 8px rgba(0,0,0,.08);background:white"><div style="flex:0 0 auto">'+icon+'</div><div style="flex:1;min-width:0"><div style="font-size:15px;font-weight:600;color:#1F2937;margin-bottom:4px">'+esc(f.display_name)+'</div><div style="font-size:12px;color:#999">'+f.type.toUpperCase()+' · '+fmtSize(f.size)+'</div></div><div style="flex:0 0 auto"><a href="'+f.download_url+'" class="editor-btn editor-btn-primary" download style="display:inline-block;padding:10px 24px;background:#1E90FF;color:#fff;border:none;border-radius:8px;text-decoration:none;font-weight:600">⬇ 下载</a></div></div>';
    }else{
      var ic=(f.kind==='zip')?'📦':'📄';
      html='<div class="media-card file-card" style="margin:12px 0;display:flex;align-items:center;gap:12px;padding:12px;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,.08);background:white"><div style="flex:0 0 auto;font-size:32px">'+ic+'</div><div style="flex:1;min-width:0"><div style="font-size:14px;font-weight:500;color:#1F2937">'+esc(f.display_name)+'</div><div style="font-size:11px;color:#999">'+f.type.toUpperCase()+' · '+fmtSize(f.size)+'</div></div><div style="flex:0 0 auto"><a href="'+f.download_url+'" class="editor-btn editor-btn-primary" download style="display:inline-block;padding:10px 20px;background:#1E90FF;color:#fff;border:none;border-radius:8px;text-decoration:none;font-weight:600;font-size:13px">⬇ 下载</a></div></div>';
    }
  ...
  function insertAtCursor(html){
    ...
    if(sel&&sel.rangeCount>0&&editor.contains(sel.anchorNode)){
      ...
      while(div.firstChild)editor.appendChild(div.firstChild);
    }
    updateStatus();
  }

  // --- 分享链接弹窗 ---
  document.getElementById('btn-copy-share').onclick=function(){var inp=document.getElementById('share-url');inp.select();try{document.execCommand('copy');toast('已复制链接','success');}catch(e){toast('复制失败','warning');}};
